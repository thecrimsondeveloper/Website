export async function loadScene(url) {
  ...
  if (scene.schema !== "crimson-harbor/world/3") throw new Error("The harbor scene schema is unsupported.");
  if (!scene.camera || !scene.water || !scene.terrain || !scene.curves || !scene.placements) throw new Error("The harbor scene is incomplete.");
